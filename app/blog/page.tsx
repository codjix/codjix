...
var page = async ({ searchParams }) => {
  ...
  const posts = await pb
    .collection("posts")
    .getList(page, parseInt(process.env.POSTS_PER_PAGE ?? "10"), {
      filter: "draft = false",
        ...
        <Box className={classes.posts} pos="relative">
          <Suspense fallback={<LoadingOverlay h={400} visible />}>
            {posts?.items.map((post, index) => {
              ...
              const banner =
                process.env.POCKETBASE + `/api/files/posts/${post.id}/${post.banner}?thumb=0x200f`;
              const avatar =
                process.env.POCKETBASE +
                `/api/files/users/${author.id}/${author.avatar}?thumb=0x50f`;
              return (
