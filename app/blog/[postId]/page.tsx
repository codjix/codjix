...
export async function generateMetadata({ params }): Promise<Metadata> {
  ...
  return mkMetaData({
    ...
    description:
      post.title ??
      "Can not get requested post. Please visit the other pages to learn more about me.",
    images: [{ url: banner }],
  });
}
...
const page = async ({ params }) => {
  ...
  const banner = process.env.POCKETBASE + `/api/files/posts/${post.id}/${post.banner}?thumb=0x200f`;
  const avatar =
    process.env.POCKETBASE + `/api/files/users/${author.id}/${author.avatar}?thumb=0x50f`;
  if (post.draft) {
    return (
      <EmptyBox
